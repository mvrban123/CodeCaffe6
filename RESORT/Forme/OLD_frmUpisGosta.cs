...
{
    public partial class old_frmUpisGosta : Form
    {
        public old_frmUpisGosta()
        {
            InitializeComponent();
        }
        ...
        private void btnDodajNaPopis_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < broj_nocenja; i++)
            {
                ...
                int zadnji = dgv.RowCount;

                //**********************************OVDJE ULAZI AKO PRVI PUT PUNI DATAGRID ILI AKO JE NASTALA NOVA CIJENA***********************************************

                if (zadnji == 0 || dgv.Rows[zadnji - 1].Cells["cijena_sobe_dgv"].FormattedValue.ToString() != cijena_sobe.ToString())
                {
                    string d_od = zadnji == 0 ? dtpDatDolaska.Value.ToString() : dat.ToString();
                    ukupno_stavka = 0;
                    ukupno_stavka = ((cijena_sobe - (((cijena_sobe) * rabat_stavka) / 100)) + vrsta_usluge + bor_pristojba);
                ...
                {
                    dgv.Rows[zadnji - 1].Cells["datum_odlaska"].Value = dat.ToString();
                    dgv.Rows[zadnji - 1].Cells["ukupno"].Value = ukupno_stavka;
                }
                dat = dat.AddDays(1);
                RacunajUkupno();
            }//for
        ...
        private void cbSoba_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (LoadCompleted)
            {
                ...
                if (DTt.Rows.Count > 0)
                {
                    cijena_sobe = Convert.ToDecimal(DTt.Rows[0]["cijena_nocenja"].ToString()) * Convert.ToDecimal(DTt.Rows[0]["tecaj"].ToString());
                }
                else
                {
                    MessageBox.Show("Za današnji dan nemate zadanu cijenu za navedenu sobu.");
        ...
        private void RacunajUkupno()
        {
            ...
            {
                sve_ukupno = Convert.ToDecimal(dgv.Rows[i].Cells["ukupno"].FormattedValue.ToString()) + sve_ukupno;
            }
        ...
        private void btnUrediOznacenog_Click(object sender, EventArgs e)
        {
            ...
            if (editDgv)
            {
                ...
                dgv.Rows[b].Cells["cijena_sobe_dgv"].Value = cijena_sobe;
                ...
                ukupno_stavka = ukupno_stavka * broj_nocenja;
                dgv.Rows[b].Cells["ukupno"].Value = ukupno_stavka.ToString("#0.00");
        ...
        private void txtBrojDokumenta_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                DataTable DT = RemoteDB.select("SELECT broj FROM unos_gosta WHERE broj='" + txtBrojDokumenta.Text + "'", "unos_gosta").Tables[0];
        ...
        private void Fill()
        {
            ...
            string sql = "SELECT * FROM unos_gosta WHERE broj='" + txtBrojDokumenta.Text + "'";
            ...
            foreach (DataRow dr in DT.Rows)
            {
                try
                {
                    ...
                    dgv.Rows[br].Cells["ukupno"].Value = Convert.ToDecimal(dr["ukupno"].ToString()).ToString("#0.00");
